            ...
            <div class="justify-center" style="margin-top: 40px">
                ...
                @if (@$disciplinas)
                    ...
                    @foreach($disciplinas as $disciplina)
                        <h6>URL de PDF: <a href="{{$url_site}}/pdf/{{$disciplina->DisciplinaID}}/{{$matriculaID}}" target="_blank">{{$url_site}}/pdf/{{$disciplina->DisciplinaID}}/{{$matriculaID}}</a></h6><br>
                            ...
                            @foreach($aula as $value)
                                @if (isset($value->DisciplinaID))
                                    @if($value->DisciplinaID == $disciplina->DisciplinaID)
                                        ...
                                        <h6>URL do video: <a href="{{$url_site}}/video/{{$value->AulaID}}/{{$value->MatriculaID}}" target="_blank">{{$url_site}}/video/{{$value->AulaID}}/{{$value->MatriculaID}}</a></h6><br><br>
                                    @endif                      
                                @endif
                            @endforeach
                        @endforeach
